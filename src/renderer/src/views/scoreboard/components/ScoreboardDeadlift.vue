...
function getAthletes() {
  window.streamData.getDeadliftScoreboard().then((entries) => {
    window.streamSettings.get().then((settings) => {
      const mapped = entries.map((entry) => mapScoreboardEntry(entry))
      athletes.value = padToSize(mapped, emptyLine, settings.deadliftScoreboardSettings.pageSize)
    })
  })
}

function mapScoreboardEntry(scoreboardEntry: ScoreboardEntryDeadlift): ScoreboardEntry {
  return {
